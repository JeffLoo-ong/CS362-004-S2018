...
int main()
{
	...
	for(x = 0; x < iterations; x++)
	{
		...
		if (OriginalS.handCount[player] + 3 != S.handCount[player])
			handFailCount++;

		//fails 3
		if (OriginalS.deckCount[player] - 3  != S.deckCount[player])
			deckFailCount++;
	...
	testFailure = totalCardsFailCount + handFailCount + deckFailCount + victoryPileFailCount + kingCardsFailCount;
